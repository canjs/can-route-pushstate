...
function makeTest(mapModuleName){
	...
	if (typeof steal !== 'undefined') {
		...
		test("Click anchor inside svg with xlink:href attribute (#81)", function() {
			stop();
			...
			makeTestingIframe(function(info, done){
				...
				info.route.register("{type}/{id}");

				info.route.start();

				// creating link that we'll test the prevent default
				var a = win.document.createElementNS("http://www.w3.org/2000/svg", "a");
				a.setAttributeNS("http://www.w3.org/2000/svg", "xlink:href", "/books/40");
				a.innerHTML = "Click Me";
				win.document.body.appendChild(a);
				domEvents.dispatch(a, "click");
				setTimeout(function() {
					equal(win.location.pathname, '/books/40');
					start();
					done();
				}, 30);
		...
		test("Click anchor inside svg with href attribute (#81)", function() {
			stop();
			...
			makeTestingIframe(function(info, done){
				...
				// creating link that we'll test the prevent default
				var a = win.document.createElementNS("http://www.w3.org/2000/svg", "a");
				a.setAttributeNS("http://www.w3.org/2000/svg", "href", "/books/40");
				a.innerHTML = "Click Me";
				win.document.body.appendChild(a);
				domEvents.dispatch(a, "click");
				setTimeout(function() {
